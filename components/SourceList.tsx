...
export function SourceList({ sources }: { sources: SourceRow[] }) {
  ...
  return (
    <ul className="space-y-2">
      {sources.map((s) => (
        <li key={s.id} className="rounded-lg border border-hairline bg-surface p-2.5">
          <a
            href={s.url}
            target="_blank"
          ...
          <p className="mt-0.5 truncate text-[11px] text-faint" title={s.url}>{s.url}</p>
          <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-faint">
            <span className="rounded bg-ink/6 px-1.5 py-0.5">{s.source_type}</span>
